...
class CDRParser:
    ...
    def parse_binary_data(self, data):
        ...
        if not records:
            ...
            self.logger.info("No ASN.1 records found, using enhanced BCD phone extraction")
                ...
                try:
                    for idx, component in enumerate(asn1_object):
                        if hasattr(asn1_object, 'componentType'):
                            ...
                            result[comp_name or f'component_{idx}'] = self.asn1_to_dict(component)
                        else:
    ...
    def determine_record_type(self, data):
        ...
        if any(indicator in data_str for indicator in ['voice', 'call', 'speech', 'gsm', 'circuit']):
            return 'voice_call'
        
        # SMS indicators
        elif any(indicator in data_str for indicator in ['sms', 'message', 'short', 'text']):
            return 'sms'
        
        # Data session indicators
        elif any(indicator in data_str for indicator in ['data', 'gprs', 'pdp', 'internet', 'packet']):
            return 'data_session'
    ...
    def find_next_asn1_start(self, data, start_offset):
        ...
        for i in range(start_offset, len(data)):
            ...
            if data[i] in [0x30, 0x31, 0x02, 0x04, 0x05, 0x06, 0x0A, 0x80, 0x81, 0x82, 0x83]:
                return i
        return -1
    ...
    def parse_raw_binary(self, data):
        ...
        if phone_matches:
            record['potential_phone_numbers'] = [match.decode('ascii', errors='ignore') for match in phone_matches[:10]]
            ...
            with open(filepath, 'rb') as f:
                while True:
                    ...
                    if len(records) > 10000:
                        self.logger.warning(f"Limiting parsing to first 10000 records for performance")
                        break
    ...
    def parse_binary_data_chunk(self, data, start_record_index=0):
        ...
        while offset < len(data) and len(records) < max_records_per_chunk and consecutive_failures < max_consecutive_failures:
            try:
                # Try to decode ASN.1 structure
                ...
                decode_chunk = remaining[:min(10000, len(remaining))]  # Max 10KB per decode attempt
    ...
    def parse_raw_binary_file(self, filepath):
        ...
        try:
            ...
            num_records = 100 if bcd_phones else 50
            for i in range(num_records):
                ...
                if network_patterns:
                    networks = [p.decode('ascii', errors='ignore') for p in network_patterns[:5]]
    ...
    def parse_telecom_binary_data(self, data):
        ...
        network_elements = [p.decode('ascii', errors='ignore') for p in network_patterns[:5]]
    ...
    def extract_bcd_phone_numbers(self, data):
        ...
        for i in range(0, len(data) - 5, 1):
            ...
            try:
                ...
                if (valid_bcd and len(number) >= 10 and len(number) <= 15 and 
                    number.isdigit() and number not in bcd_numbers):
                    
                    # Additional validation for realistic phone numbers
                    if (number.startswith('91') and len(number) >= 12 or  # Indian international
                        (len(number) == 10 and number[0] in '6789') or  # Indian mobile
                        (len(number) >= 11 and number[0] != '0')):  # International format
